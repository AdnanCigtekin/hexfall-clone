        ...
        void CreateNewCellsAtTop()
        {
            ...
            if (sV.cellQueue.Count != 0)
            {
                ...
                foreach (KeyValuePair<CellProperty, MyGrid> item in sV.cellQueue)
                {
                    ...
                    {
                        item.Key.bombAlerter.enabled = true;
                    ...
                    {
                        MyGrid temp = item.Key.GetAnyTopGrid();
                        if (temp != null)
                        {
                            item.Key.bombAlerter.enabled = true;
                            item.Key.GetComponent<SpriteRenderer>().enabled = true;
                            item.Key.currentGrid = temp;
                            temp.assignedCell = item.Key;
                            item.Key.transform.position = temp.transform.position;
                            item.Key.targetPos = temp.transform.position;
                            item.Key.markedForDestruction = false;
                            keys.Add(item.Key);
                        }
                    }
